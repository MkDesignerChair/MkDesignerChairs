...
function createDefaultChairVariants(basePrice: number, colorNames: string[]): ProductVariantInput[] {
  const standardVariant = { variant_name: 'Standard', price: basePrice, original_price: null, stock_quantity: 0, is_active: true }

  if (colorNames.length === 0) return [standardVariant]

  return colorNames.map(colorName => ({
    ...standardVariant,
    variant_name: `${colorName} - Standard`,
  }))
}
...
export async function createProduct(
  ...
  const chairVariants = createDefaultChairVariants(basePrice, colorNames)

  await supabase.from('product_variants').insert(
    chairVariants.map(v => ({ product_id: id, ...v }))
  )
