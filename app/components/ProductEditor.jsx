...
export default function ProductEditor({ categories, product, onClose, onDeleted, onSaved }) {
  const isNewProduct = !product;
  const [draft, setDraft] = useState(product || { name: "", price: "", categoryId: categories[0]?.id || "", image: "", stock: 0, isActive: true, featured: false, galleryImages: [] });
  ...
  async function uploadPrimaryImage(file) {
    ...
    try {
      ...
      setDraft((current) => ({ ...current, id: isNewProduct ? uploadedImage.id : current.id, image: uploadedImage.image }));
  ...
  async function uploadGalleryImages(files) {
    ...
    try {
      ...
      for (const file of filesToUpload) uploadedImages.push((await uploadFile(file)).image);
  ...
  async function save(event) {
    ...
    if (!draft.id || !draft.image) {
      setMessage("Upload a primary product image before saving.");
      return;
    }
    setIsSaving(true);
    setMessage("");
    ...
    onSaved({ ...draft, ...result.override, id: draft.id });
    onClose();
  }

  async function removeProduct() {
    if (!window.confirm(`Delete ${product.name}? This removes it from the storefront and catalog.`)) return;

    setIsSaving(true);
    setMessage("");
    try {
      const response = await fetch("/api/admin/catalog", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: product.id }) });
      const result = await readApiResponse(response);
      if (!response.ok) throw new Error(result.error || "Unable to delete the product.");
      onDeleted(product.id);
      onClose();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to delete the product.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return <section className="admin-product-editor">
    <div className="admin-page-heading"><div><p className="admin-kicker">CHAIR CATALOG</p><h1>{isNewProduct ? "Add product" : "Edit product"}</h1><p>{isNewProduct ? "Create a new chair with its primary image and customer-facing details." : `Update all customer-facing information for ${product.name}.`}</p></div><button className="admin-site-link admin-editor-back" type="button" onClick={onClose}>← Back to products</button></div>
    <form onSubmit={save}>
      ...
      <div className="admin-category-editor-actions"><button className="admin-save-product" disabled={isSaving} type="submit">{isSaving ? "Saving…" : isNewProduct ? "Create product" : "Save product changes"}</button>{!isNewProduct && <button className="admin-delete-category" disabled={isSaving} type="button" onClick={removeProduct}>Delete product</button>}</div>
    </form>
  </section>;
}
