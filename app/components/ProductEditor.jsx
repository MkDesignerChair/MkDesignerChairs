...
export default function ProductEditor({ product, onClose, onSaved }) {
  ...
  return <section className="admin-product-editor">
    <div className="admin-page-heading"><div><p className="admin-kicker">CHAIR CATALOG</p><h1>Edit product</h1><p>Update all customer-facing information for {product.name}.</p></div><button className="admin-site-link" type="button" onClick={onClose}>← Back to products</button></div>
    <form onSubmit={save}>
      <div className="admin-editor-grid">
        <section className="admin-editor-card"><h2>Basic information</h2><label>Product name<input required value={draft.name} onChange={(event) => setField("name", event.target.value)} /></label><div className="admin-editor-two"><label>Category<select value={draft.category} onChange={(event) => setField("category", event.target.value)}>{categories.map((category) => <option key={category}>{category}</option>)}</select></label><label>Price (₹)<input min="1" required type="number" value={draft.price} onChange={(event) => setField("price", event.target.value)} /></label></div><label>Short description<textarea value={draft.shortDescription || ""} onChange={(event) => setField("shortDescription", event.target.value)} /></label><label>Full description<textarea rows="6" value={draft.description || ""} onChange={(event) => setField("description", event.target.value)} /></label></section>
        <section className="admin-editor-card"><h2>Image & availability</h2><img className="admin-editor-image" src={draft.image} alt={draft.name} /><label className="admin-file-input">Replace primary image<input accept="image/jpeg,image/png,image/webp" disabled={isSaving} type="file" onChange={(event) => uploadPrimaryImage(event.target.files?.[0])} /></label><small>JPG, PNG, or WEBP · max 5 MB</small><label>Stock quantity<input min="0" type="number" value={draft.stock || 0} onChange={(event) => setField("stock", event.target.value)} /></label><label>Badge / tag<input placeholder="Best seller, New arrival" value={draft.badge || ""} onChange={(event) => setField("badge", event.target.value)} /></label><label className="admin-check"><input checked={draft.isActive !== false} type="checkbox" onChange={(event) => setField("isActive", event.target.checked)} />Visible in storefront</label><label className="admin-check"><input checked={Boolean(draft.featured)} type="checkbox" onChange={(event) => setField("featured", event.target.checked)} />Feature on homepage</label></section>
