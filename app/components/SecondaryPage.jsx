...
function NavigationScrollProgress() {
  ...
  useEffect(() => {
    ...
    function updateProgress() {
      ...
      const progress = maximumScroll > 0 ? Math.min(Math.max(window.scrollY / maximumScroll, 0), 1) : 0;
  ...
  return <span aria-hidden="true" className="nav-scroll-progress" ref={progressRef} />;
}
...
export function SiteNavigation({ active, categories = [] }) {
  ...
  useEffect(() => {
    ...
    async function loadCustomerSession() {
      ...
      if (activeSession) setCustomer(result.user || null);
    }
  ...
  function returnToCurrentPageTop(event, href) {
    const [targetPath, targetSearch = ""] = href.split("?");
    if (targetPath !== window.location.pathname || targetSearch !== window.location.search.slice(1)) return;
  ...
  return (
    <nav className="inner-navbar" aria-label="Main navigation">
      <Link className="brand" href="/" aria-label="Designer Chairs home" onClick={(event) => returnToCurrentPageTop(event, "/")}><Image className="brand-logo" src={logo} alt="MK Designer Chairs" width={154} priority /></Link>
      <div className="inner-nav-links">{links.map((link) => <Link className={link.label === active || link.href === active ? "active" : ""} href={link.href} key={link.href} onClick={(event) => returnToCurrentPageTop(event, link.href)} scroll>{link.label}</Link>)}</div>
...
export default function SecondaryPage({ active, eyebrow, title, description, image, children, ctaText = "Request a Quote", quoteHref = "/contact" }) {
  return (
    <main className="inner-page">
      <section className="inner-hero">
        ...
        <div className="inner-hero-copy"><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p>{description}</p><Link className="gold-button" href={quoteHref}>{ctaText}</Link></div>
      </section>
      {children}
    </main>
  );
